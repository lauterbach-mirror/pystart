...
class PowerView:
    ...
    Args:
        connection: Specifies how to communicate with the target. If set to ``None`` connection gets configured by
            ``connection_script`` or by user.
    ...
    def get_configuration_string(self) -> str:
        ...
        if self._connection is not None:
            fragments.append(self._connection._get_config_string(self))
        fragments.extend(
            [self._get_config_string_license(), self._get_config_string_screen(), self._get_config_string_interface()]
        )
